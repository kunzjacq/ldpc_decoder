...
const bool g_use_pinned_memory = false;
...
ldpc_decoder_gpu::ldpc_decoder_gpu(const ldpc_code &p_code, const noisy_channel &p_channel,
    ...
    : m_llrs(nullptr), m_message_buffer(nullptr), m_max_table_index(0), m_noise_factor(), m_n_edges(p_code.n_edges()),
      ...
      m_convert_llr_bsc_k(nullptr), m_convert_llr_biawgn_k(nullptr), m_flood_backward_k(nullptr),
      m_flood_forward_k(nullptr), m_flood_forward_w_final_bits_k(nullptr), m_deinterlace_output_k(nullptr),
      m_check_parity_k(nullptr), m_refill_k(nullptr), m_refill_vec_k(nullptr), m_permute_k(nullptr),
      ...
      m_log2_parallel_factor(0), m_parallel_factor(1), m_channel(p_channel) {
  if (m_n_inputs & 0x1F) {
    throw error("This decoder only handles input sizes that are multiple of 32");
  }

  if (!m_device_manager.m_initialized) {
    cout << "Error when initializing OpenCL. Exiting." << endl;
    exit(1);
  }

  if (p_params.m_max_log_parallel_factor_user < log2_simd_parallel_factor) {
    cout << "User asks for processing at most 2**" << p_params.m_max_log_parallel_factor_user
         << " vectors in parallel, but implementation has a SIMD size of 2**" << log2_simd_parallel_factor
         << ", constraint cannot be satisfied." << endl;
    exit(1);
  }

  // load openCL kernels
  try {
    m_device_manager.build_program(string(m_program_name));
    m_convert_llr_bsc_k = m_device_manager.create_kernel("llr_bsc");
    m_convert_llr_biawgn_k = m_device_manager.create_kernel("llr_biawgn");
    m_flood_forward_k = m_device_manager.create_kernel("flood_forward");
    m_flood_forward_w_final_bits_k = m_device_manager.create_kernel("flood_forward_w_final_bits");
    m_flood_backward_k = m_device_manager.create_kernel("flood_backward");
    m_deinterlace_output_k = m_device_manager.create_kernel("deinterlace_output");
    m_check_parity_k = m_device_manager.create_kernel("check_parity");
    m_refill_k = m_device_manager.create_kernel("flood_refill");
    m_refill_vec_k = m_device_manager.create_kernel("flood_refill_vec");
    m_permute_k = m_device_manager.create_kernel("flood_permute_vecs");
  } catch (exception &e) {
    cout << e.what() << endl;
    exit(EXIT_FAILURE);
  }
  ...
  cl_ulong total_memory = m_device_manager.get_total_global_memory();
  ...
  cl_ulong instance_memory =
      2 * (m_n_outputs >> 3) + sizeof(transfer_llr_t) * m_n_edges + (2 * sizeof(transfer_llr_t) + 1) * m_n_inputs + (m_n_inputs >> 3);
  ...
  cl_ulong max_parallel_factor_from_gpu = (total_memory - security_memory - code_repr_memory) / instance_memory;
  ...
  m_log2_threads_per_simd_vector = m_log2_global_threads - m_log2_parallel_factor + log2_simd_parallel_factor;
  ...
  if (g_use_pinned_memory) {
    ...
    m_device_manager.create_host_pinned_buffer(cl_manager::w, n_inputs_p, m_in_mapped_dev_buf, m_llrs);
    m_device_manager.create_host_pinned_buffer(cl_manager::r, input_uint32_sz * m_parallel_factor,
                                           m_out_packed_mapped_dev_buf, m_final_bits_packed);
  } else {
    m_llrs = new transfer_llr_t[n_inputs_p];
    m_final_bits_packed = new uint32_t[input_uint32_sz * m_parallel_factor];
  }

  m_syndrome_dev_buf = m_device_manager.create_device_buffer<uint32_t>(cl_manager::w, m_n_outputs_words_p);
  m_new_syndrome_dev_buf = m_device_manager.create_device_buffer<uint32_t>(cl_manager::w, m_n_outputs_words_p);
  m_message_dev_buf = m_device_manager.create_device_buffer<transfer_llr_t>(cl_manager::r, n_edges_p);
  m_initial_llrs_dev_buf = m_device_manager.create_device_buffer<transfer_llr_t>(cl_manager::w, n_inputs_p);
  m_new_initial_llrs_dev_buf = m_device_manager.create_device_buffer<transfer_llr_t>(cl_manager::w, n_inputs_p);
  m_final_bits_dev_buf = m_device_manager.create_device_buffer<char>(cl_manager::r, n_inputs_p);
  m_final_bits_packed_dev_buf =
      m_device_manager.create_device_buffer<uint32_t>(cl_manager::r, input_uint32_sz * m_parallel_factor);
  m_vec_places_dev_buf = m_device_manager.create_device_buffer<uint32_t>(cl_manager::r, m_parallel_factor);
  m_parities_violated_dev_buf = m_device_manager.create_device_buffer<char>(cl_manager::r, m_parallel_factor);
  m_swap_o_dev_buf = m_device_manager.create_device_buffer<uint32_t>(cl_manager::w, m_parallel_factor);
  m_swap_d_dev_buf = m_device_manager.create_device_buffer<uint32_t>(cl_manager::w, m_parallel_factor);

  m_device_manager.create_device_buffer_from_host_data(cl_manager::r, m_n_outputs + 1, m_out_bit_to_edge_dev_buf,
                                                   m_out_bit_to_edge);
  m_device_manager.create_device_buffer_from_host_data(cl_manager::r, m_n_edges, m_out_to_in_edge_dev_buf,
                                                   m_out_to_in_edge);
  m_device_manager.create_device_buffer_from_host_data(cl_manager::r, m_n_edges, m_in_to_out_edge_dev_buf,
                                                   m_in_to_out_edge);
  m_device_manager.create_device_buffer_from_host_data(cl_manager::r, m_n_inputs + 1, m_in_bit_to_edge_dev_buf,
                                                   m_in_bit_to_edge);
  m_device_manager.create_device_buffer_from_host_data(cl_manager::r, m_n_edges, m_out_edge_to_in_bit_dev_buf,
                                                   m_out_edge_to_in_bit);
}

ldpc_decoder_gpu::~ldpc_decoder_gpu() {
  m_device_manager.release_kernel(m_convert_llr_bsc_k);
  m_device_manager.release_kernel(m_convert_llr_biawgn_k);
  m_device_manager.release_kernel(m_flood_forward_k);
  m_device_manager.release_kernel(m_flood_forward_w_final_bits_k);
  m_device_manager.release_kernel(m_flood_backward_k);
  m_device_manager.release_kernel(m_deinterlace_output_k);
  m_device_manager.release_kernel(m_check_parity_k);
  m_device_manager.release_kernel(m_refill_k);
  m_device_manager.release_kernel(m_refill_vec_k);
  m_device_manager.release_kernel(m_permute_k);
 ...
 */
void ldpc_decoder_gpu::prepare_vectors(transfer_llr_t *p_input, uint32_t p_input_stride, uint32_t p_output_stride,
...
void ldpc_decoder_gpu::transfer_vectors(uint32_t p_num_vectors, transfer_llr_t *p_llrs, const uint32_t *p_syndromes,
                                        float p_threshold) {
  const int64_t n_regular_variables = m_n_inputs - m_n_erased_inputs;
  ...
  m_device_manager.enqueue_write(m_new_initial_llrs_dev_buf, CL_FALSE, 0, bound_r, p_llrs, 0, nullptr, nullptr);
  // transfer syndromes to GPU
  m_device_manager.enqueue_write(m_new_syndrome_dev_buf, CL_FALSE, 0, m_syndrome_uint32_sz * p_num_vectors, p_syndromes, 0,
                             nullptr, nullptr);
  m_device_manager.enqueue_barrier();
  // clear LLRs corresponding to erased variables
  m_device_manager.enqueue_clear<float>(m_new_initial_llrs_dev_buf, bound_r, bound_e, 0, nullptr, nullptr);
  ...
  if (c == bsc || c == awgn) {
    ...
    list<cl_manager::arg_k> arg_list_llr(
        {cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_new_initial_llrs_dev_buf)),
         cl_manager::arg_k(sizeof(transfer_llr_t), static_cast<const void *>(&m_noise_factor)),
    ...
    if (c == bsc) {
      ...
      assert(cc);
      m_noise_factor = cc->ref_llr();
      // sets arguments before enqueuing call because m_noise_factor is modified
      m_device_manager.enqueue_call_with_args(m_convert_llr_bsc_k, arg_list_llr, 1, &m_local_threads, &m_global_threads, 0,
                                          nullptr, nullptr);
    } else if (c == awgn) {
      auto cc = dynamic_cast<const biawgn_channel *>(&m_channel);
      assert(cc);
      m_noise_factor = cc->factor();
      // sets arguments before enqueuing call because m_noise_factor is modified
      m_device_manager.enqueue_call_with_args(m_convert_llr_biawgn_k, arg_list_llr, 1, &m_local_threads, &m_global_threads,
                                          0, nullptr, nullptr);
    }
  }
  m_device_manager.enqueue_barrier();
  ...
  list<cl_manager::arg_k> arg_list_refill(
      {cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_message_dev_buf)),  
       ...
       cl_manager::arg_k(sizeof(transfer_llr_t), static_cast<const void *>(&p_threshold)),
  ...
  for (int i = 31; i >= (int)ldpc_decoder_gpu::log2_simd_parallel_factor; i--) {
    if ((1u << i) & p_num_vectors) {
      ...
      m_device_manager.set_args(m_refill_vec_k, arg_list_refill);
      m_device_manager.enqueue_call(m_refill_vec_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
      offset += 1u << i;
    }
  }
#else
  for (int i = 31; i >= 0; i--) {
    if ((1u << i) & p_num_vectors) {
      log2_num_new_vectors = i;
      m_cl_manager.set_args(m_refill_k, arg_list_refill);
      m_cl_manager.enqueue_call(m_refill_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
      offset += 1u << i;
    }
  }
...
void ldpc_decoder_gpu::decode(const ldpc_decoder_gpu_dynamic_parameters &p_dyn_params,
  ...
  transfer_llr_t *p_input_llrs = static_cast<transfer_llr_t *>(p_input_llrs_void);
  ...
  list<cl_manager::arg_k> arg_list_forward(
      {cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_message_dev_buf)),
       ...
       cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_in_to_out_edge_dev_buf)),
       cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_in_bit_to_edge_dev_buf)),
       cl_manager::arg_k(sizeof(transfer_llr_t), static_cast<const void *>(&p_dyn_params.m_infinity_threshold)),
       cl_manager::arg_k(sizeof(uint32_t), static_cast<const void *>(&m_log2_parallel_factor)),
       cl_manager::arg_k(sizeof(int64_t), static_cast<const void *>(&m_n_inputs)),
       cl_manager::arg_k(sizeof(int32_t), static_cast<const void *>(&m_log2_global_threads))});
  ...
  list<cl_manager::arg_k> arg_list_deinterlace_output(
      {cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_final_bits_dev_buf)),
       ...
       cl_manager::arg_k(sizeof(cl_mem), static_cast<void *>(&m_parities_violated_dev_buf)),
       cl_manager::arg_k(sizeof(transfer_llr_t), static_cast<const void *>(&m_log2_parallel_factor)),
       cl_manager::arg_k(sizeof(int64_t), static_cast<const void *>(&m_n_inputs)),
       cl_manager::arg_k(sizeof(int32_t), static_cast<const void *>(&m_log2_threads_per_simd_vector))});
  ...
  m_device_manager.set_args(m_flood_forward_k, arg_list_forward);
  m_device_manager.set_args(m_flood_forward_w_final_bits_k, arg_list_forward_w_final_bits);
  m_device_manager.set_args(m_flood_backward_k, arg_list_backward);
  m_device_manager.set_args(m_deinterlace_output_k, arg_list_deinterlace_output);
  m_device_manager.set_args(m_check_parity_k, arg_list_check_parity);
  ...
  transfer_vectors(num_vectors_gpu_batch, m_llrs, p_syndromes, p_dyn_params.m_infinity_threshold);
  ...
  while (true) {
    m_device_manager.enqueue_call(m_flood_backward_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
    m_device_manager.enqueue_barrier();

    const bool do_parity_check = (global_iter > 0) && ((global_iter % p_dyn_params.m_num_iter_check_parity) == 0);

    if (!do_parity_check) {
      m_device_manager.enqueue_call(m_flood_forward_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
      m_device_manager.enqueue_barrier();
    } else {
      if (p_log >= 1) {
      ...
      m_device_manager.enqueue_call(m_flood_forward_w_final_bits_k, 1, &m_local_threads, &m_global_threads, 0, nullptr,
                                nullptr);
      m_device_manager.enqueue_barrier();
      m_device_manager.enqueue_call(m_check_parity_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
      m_device_manager.enqueue_barrier();
      m_device_manager.enqueue_read(m_parities_violated_dev_buf, CL_FALSE, 0, m_parallel_factor, m_parity_violations, 0,
                                nullptr, nullptr);
      m_device_manager.finish();
      ...
      if (p_log >= 3)
        cout << " vectors that should be retrieved (marked with *):" << endl;
      ...
      if (next_vector_to_load == p_num_vectors_to_process && num_vectors_to_stop == num_vectors_gpu_batch) {
        ...
        m_device_manager.enqueue_call(m_deinterlace_output_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
        m_device_manager.enqueue_barrier();

        // retrieve the packed representation of all remaining vectors in gpu
        m_device_manager.enqueue_read(m_final_bits_packed_dev_buf, CL_FALSE, 0, input_uint32_sz * num_vectors_gpu_batch,
      ...
      if (num_new_vectors > 0) {
        ...
        if (num_swaps > 0) {
          m_device_manager.enqueue_write(m_swap_o_dev_buf, CL_FALSE, 0, num_swaps, &origin[0], 0, nullptr, nullptr);
          m_device_manager.enqueue_write(m_swap_d_dev_buf, CL_FALSE, 0, num_swaps, &dest[0], 0, nullptr, nullptr);
          m_device_manager.enqueue_barrier();

          log2_num_swaps = 0;
          while ((1u << log2_num_swaps) < num_swaps)
            log2_num_swaps++;
          m_device_manager.set_args(m_permute_k, arg_list_permute);
          m_device_manager.enqueue_call(m_permute_k, 1, &m_local_threads, &m_global_threads, 0, nullptr, nullptr);
          m_device_manager.enqueue_barrier();
        }
        ...
        m_device_manager.enqueue_read(m_final_bits_packed_dev_buf, CL_FALSE, 0, input_uint32_sz * num_new_vectors,
                                  m_final_bits_packed, 0, nullptr, nullptr);
        m_device_manager.finish();
